# Import CN product demo v2 (DJI Mini 5 Pro) into GEOFlow knowledge base
...
$packDir = Join-Path $root "docs\pilot-demo\cn-product-demo-v2"
$factCards = Join-Path $packDir "fact-cards.md"
$metricsPath = Join-Path $packDir "metrics.json"
$distMetrics = Join-Path $root "dist\pilot-demo\cn-product-demo-v2\metrics.json"
...
if (-not (Get-Command docker -ErrorAction SilentlyContinue)) {
    Write-Error "docker not found. See docs/pilot-demo/cn-product-demo-v2/import-to-geoflow.md"
}
...
$tmpDir = Join-Path $env:TEMP "cn-product-demo-v2-kb-import"
New-Item -ItemType Directory -Force -Path $tmpDir | Out-Null
$localPhp = Join-Path $tmpDir "import_kb.php"

$php = @'
<?php
$name = json_decode(file_get_contents('/tmp/cn-product-demo-v2-kb-name.json'), true);
$path = '/tmp/cn-product-demo-v2-fact-cards.md';
...
$payload = [
    'name' => $name,
    'description' => 'CN product demo v2 fact cards from DJI Mini 5 Pro public specs/FAQ (not official partnership). See GEORank docs/pilot-demo/cn-product-demo-v2/',
    'content' => $content,
    'file_type' => 'markdown',
    'character_count' => mb_strlen($content, 'UTF-8'),
    'word_count' => mb_strlen(strip_tags($content), 'UTF-8'),
    'source_name' => 'DJI Official Specs',
    'source_url' => 'https://www.dji.com/cn/mini-5-pro/specs',
    'source_type' => 'website',
    'business_line' => 'cn-product-demo-v2',
...
docker cp $factCards "${container}:/tmp/cn-product-demo-v2-fact-cards.md" | Out-Null
docker cp $nameFile "${container}:/tmp/cn-product-demo-v2-kb-name.json" | Out-Null
docker cp $localPhp "${container}:/tmp/import_cn_product_demo_v2_kb.php" | Out-Null

$resultJson = docker exec $container php artisan tinker --execute="require '/tmp/import_cn_product_demo_v2_kb.php';"
...
try {
    ...
    if (Test-Path $metricsPath) {
        ...
        $metrics.label = "Imported Flow KB #$($info.id); $($info.vectorized)/$($info.chunk_total) chunks vectorized"
