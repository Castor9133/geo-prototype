...
class KnowledgeDocument(Base):
    ...
    # L1 权威 / L2 故事 / L3 口径 / L4 旁路（不进生成检索）
    tier: Mapped[str] = mapped_column(String(10), default="L2", index=True)
    tags: Mapped[dict | None] = mapped_column(JSONB, default=dict)
    # pending_external | pending_local | approved | retired
    review_state: Mapped[str] = mapped_column(String(40), default="approved", index=True)
    fact_cards: Mapped[list | None] = mapped_column(JSONB, default=list)
    external_approved_at: Mapped[datetime | None] = mapped_column(DateTime)
    local_confirmed_at: Mapped[datetime | None] = mapped_column(DateTime)
    local_confirmed_by: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True))
...
class ContentTask(Base):
    ...
    # claimed → template_draft → channel_draft → in_review → ready → archived|promoted
    workflow_status: Mapped[str] = mapped_column(String(40), default="claimed", index=True)
    template_draft_body: Mapped[str | None] = mapped_column(Text)
    channel_draft_body: Mapped[str | None] = mapped_column(Text)
    channel_key: Mapped[str | None] = mapped_column(String(120))
    ...
    promoted_document_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True))
